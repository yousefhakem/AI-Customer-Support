...
import { getS3Url, getS3File } from "../lib/s3.js";
...
export async function extractTextContent(
  ...
  const { s3Key, filename, buffer, mimeType } = args;
  ...
  if (mimeType.toLowerCase().includes("text")) {
    return extractTextFileContent(s3Key, buffer, mimeType);
  }
...
async function extractTextFileContent(
  ...
  const fileBuffer = buffer || (await getS3File(s3Key));
